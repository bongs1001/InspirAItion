...
# # 수정: 올바른 환경 변수 사용
openAI_api_key = os.getenv("AZURE_OPENAI_API_KEY")  # 수정: AZURE_OPENAI_API_KEY 사용
openAI_endpoint = os.getenv("AZURE_OPENAI_ENDPOINT")  # 수정: AZURE_OPENAI_ENDPOINT 사용
...
class MetMuseumClient(BaseAPIClient):
    def __init__(self, session: aiohttp.ClientSession):
        super().__init__(
            session, "https://collectionapi.metmuseum.org/public/collection/v1"
        )
    ...
    async def _fetch_context_data(self, search_query: str) -> Any:
        ...
        try:
            async with self.session.get(
                f"https://collectionapi.metmuseum.org/public/collection/v1/search?q={search_query}"
            ) as response:
                if response.status == 200:
                    data = await response.json()
                    return data
