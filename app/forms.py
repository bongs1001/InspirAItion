    ...
    def clean(self):
        ...
        if start_time and end_time:
            if start_time < timezone.now():
                raise forms.ValidationError('경매 시작 시간은 현재 시간 이후여야 합니다.')
            if end_time <= start_time:
                raise forms.ValidationError('경매 종료 시간은 시작 시간 이후여야 합니다.')
            if end_time < timezone.now():
                raise forms.ValidationError('경매 종료 시간은 현재 시간 이후여야 합니다.')
